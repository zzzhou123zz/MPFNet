...
class KAISTMissrateMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            yy_values = eval_result['all'].eval['yy'][0] if eval_result['all'].eval['yy'] else []
            if len(yy_values) > 0:
                recall_all = 1 - yy_values[-1]
            else:
                recall_all = 0.0  # 或者其他默认值
                print("Warning: No valid evaluation results found, setting recall_all to 0.0")
