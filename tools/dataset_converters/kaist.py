...
def convert_bbox_info(label, idx, obj_count, image_height, image_width):
    ...
    # 跳过注释行和非标准行
    if not label or label[0].startswith('%') or 'bbGt' in label[0]:
        return None, obj_count

    # 确保有足够的元素
    if len(label) < 5:
        print(f"警告: 跳过格式不正确的行: {label}")
        return None, obj_count
...
def convert_yolo_to_coco(image_dir: str):
    ...
    for idx, image in enumerate(mmengine.track_iter_progress(indices)):
        ...
        with open(label_path) as f:
            labels = f.readlines()
            for label in labels:
                coco_info, new_obj_count = convert_bbox_info(
    ...
    print(f'Number of images found: {total}, converted: {converted},',
          f'skipped: {skipped}, invalid lines: {invalid_lines}.')
    print(f'Total annotation count: {obj_count}.')
