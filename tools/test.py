...
# 给出config、checkpoint即可，也可以指定保存位置
...
def parse_args():
    ...
    parser.add_argument(
        '--work-dir', default = "/data/zfy/mmyolo/work_dirs/dvtest",
...
def main():
    for i, data_batch in enumerate(data_loader):
        print(f"Batch {i} input shape: {data_batch['inputs'].shape}")
        # 确保输入是4维的
        if data_batch['inputs'].dim() == 3:
            print("Warning: 3D input detected, adding batch dimension")
            data_batch['inputs'] = data_batch['inputs'].unsqueeze(0)
    args = parse_args()
