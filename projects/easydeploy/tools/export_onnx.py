...
def main():
    ...
    fake_input1 = torch.randn(args.batch_size, 3,
                             *args.img_size).to(args.device)
    fake_input2 = torch.randn(args.batch_size, 3,
                             *args.img_size).to(args.device)
    # dry run
    deploy_model(fake_input1,fake_input2)
    ...
    with BytesIO() as f:
        torch.onnx.export(
            deploy_model,
            (fake_input1,fake_input2),
            f,
            input_names=['batch_inputs','batch_inputs2'],
