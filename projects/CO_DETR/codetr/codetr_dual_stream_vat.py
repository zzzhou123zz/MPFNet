...
class CoDETR_Dual_Vat(BaseDetector):
    ...
    def __init__(
        ...
        super(CoDETR_Dual_Vat, self).__init__(
    ...
    def add_noise(self, feats):
        feats_w_noise = []
        for feat in feats:
            feats_w_noise.append(self.noise(feat.shape, feat.device, 1e-4) + feat.detach())
    ...
    def loss(self, batch_inputs: Tensor, batch_inputs2: Tensor,
        ...
        def consistent_loss(cls_, cls_w_noise, coords_, coords_w_noise):
            new_losses = dict()
            print(cls_[0].shape, coords_[0].shape)
            return new_losses
        ...
        if self.with_query_head:
            ...
            outputs_classes, outputs_coords, topk_score, topk_anchor, outs = self.query_head(x_)
            outputs_classes_w_noise, outputs_coords_with_noise, _, _, outs_with_noise = self.query_head(x_noise)
            cons_losses = consistent_loss(outputs_classes, outputs_classes_w_noise, outputs_coord, outputs_coords_with_noises)
