...
class CoDINOHead_Vat(DINOHead):
    ...
    def loss(self, x, x_noise, batch_data_samples):
        assert self.dn_generator is not None, '"dn_cfg" must be set'

        batch_gt_instances = []
        batch_img_metas = []
        batch_img_metas_double = []
        for data_sample in batch_data_samples:
            batch_img_metas.append(data_sample.metainfo)
            # batch_img_metas.append(data_sample.metainfo)
            batch_gt_instances.append(data_sample.gt_instances)
            # batch_img_metas_double.append(data_sample.metainfo)
            # batch_img_metas_double.append(data_sample.metainfo)
            
        # batch_gt_instances_double = batch_gt_instances * 2
        batch_img_metas_double = batch_img_metas * 2
        
        dn_label_query, dn_bbox_query, attn_mask, dn_meta = \
            self.dn_generator(batch_data_samples)

        with torch.no_grad():
            outs_noise = self(x_noise, batch_img_metas, dn_label_query, dn_bbox_query,
                        attn_mask)
        x_no_grad = [i.detach() for i in x]
        
        # outs_ = self(x_no_grad, batch_img_metas, dn_label_query, dn_bbox_query,
        #             attn_mask)
        B = x[0].shape[0]
        x_combine = [
            torch.concat([i, j], dim=0)
            for i, j in zip(x, x_no_grad)
        ]
        
        outs_combine = self(x_combine, batch_img_metas_double, torch.cat([dn_label_query] * 2, dim=0), torch.cat([dn_bbox_query] * 2),
                    attn_mask)

        outs = []
        outs_ = []
        for idx, i in enumerate(outs_combine):
            if idx == 4: # enc_outputs list
                outs.append([j[:B] for j in i])
                break
        ...
        return losses, enc_outputs, outs_[:2], outs_noise[:2]
