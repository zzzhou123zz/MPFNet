...
class Dual_ResNet(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(Dual_ResNet, self).__init__(init_cfg)
        ...
        self._make_stem_layer_t(in_channels, stem_channels)
        # self.eaef1 = EAEF(256)
        # self.eaef2 = EAEF(512)
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            eaef_layers = EAEF(planes * self.block.expansion)
            
            eaef_layer_name = f'eaef{i + 1}'
            self.add_module(eaef_layer_name, eaef_layers)
            self.eaef_layers.append(eaef_layer_name)
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if plugins is not None:
                ...
                stage_plugins = None
            planes = base_channels * 2**i
            res_layers_t = self.make_res_layer_t(
                block=self.block,
                inplanes=self.inplanes,
                planes=planes,
                num_blocks=num_blocks,
                ...
                init_cfg=block_init_cfg)
            self.inplanes = planes * self.block.expansion
            layer_name_t = f't_layer{i + 1}'
            self.add_module(layer_name_t, res_layers_t)
            self.res_layers_t.append(res_layers_t)
    ...
    def make_res_layer_t(self, **kwargs):
        """Pack all blocks in a stage into a ``ResLayer``."""
        return ResLayer(**kwargs)
    ...
    def _make_stem_layer_t(self, in_channels, stem_channels):
        if self.deep_stem:
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        if self.deep_stem:
            ...
            self.conv1_t = build_conv_layer(
                ...
                padding=3,
                bias=False)
            self.norm1_name_t, norm1_t = build_norm_layer(
                self.norm_cfg, stem_channels, postfix=1)
    ...
    def forward(self, x):
        ...
        x_T = self.maxpool_t(x_T)
        outs = []
        for i, layer_name in enumerate(self.res_layers):
            res_layer = getattr(self, layer_name)
            res_layers_t = getattr(self, "t_" + layer_name )
            x_RGB = res_layer(x_RGB)
            x_T = res_layers_t(x_T)

            # x_fuse = x_RGB + x_T
            if i in self.out_indices:
                eaef_layer = getattr(self, self.eaef_layers[i])
                x_RGB, x_T, x_fuse = eaef_layer([x_RGB, x_T])[0], eaef_layer([x_RGB, x_T])[1], eaef_layer([x_RGB, x_T])[2]
                outs.append(x_fuse)
        return tuple(outs)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(Dual_ResNet, self).train(mode)
        self._freeze_stages()
