...
# class YOLOv8CSPDarknetPzconvWithDenoise(BaseBackbone):
#     """带ConvNeXt去噪分支的单分支YOLOv8 CSP-Darknet骨干网络
#     在Stage1-3输出后添加去噪分支，同时保留原始特征用于fusion
#     """
#     arch_settings = {
...
#                  out_indices: Tuple[int] = (2, 3, 4),  # 对应Stage1-3的输出
#                  frozen_stages: int = -1,
...
#                  init_cfg=None,
#                  # 去噪模块配置
#                  use_denoise: bool = True,  # 是否启用去噪分支
#                  denoise_depths: list = [2, 2, 2],  # 去噪模块的深度
#                  denoise_dims: list = [48, 96, 192],):  # 去噪模块的通道数
#         self.use_pzconv_in_stage = True  # 沿用原Pzconv配置
#         self.use_pzconv_in_csp = False
#         self.arch_settings[arch][-1][1] = last_stage_out_channels
#         self.use_denoise = use_denoise  # 控制是否使用去噪分支
#
#         super().__init__(
...
#             init_cfg=init_cfg)
#
#         # 为每个输出Stage创建ConvNeXt去噪模块（Stage1-3对应3个去噪模块）
#         if self.use_denoise:
#             self.denoise_modules = nn.ModuleList()
#             # 计算每个Stage输出的实际通道数（用于匹配去噪模块输入）
#             stage_out_channels = [
#                 make_divisible(self.arch_settings[arch][i][1], widen_factor)
#                 for i in range(len(self.out_indices))
#             ]
#             # 初始化3个去噪模块（分别对应Stage1-3的输出特征）
#             for i in range(len(self.out_indices)):
#                 denoise_module = LightweightDenoiseConvNeXt(
#                     in_channels=stage_out_channels[i],  # 输入通道=对应Stage的输出通道
#                     depths=denoise_depths,
#                     dims=denoise_dims
#                 )
#                 self.denoise_modules.append(denoise_module)
#
...
#     def forward(self, x):
#         """
#         带去噪分支的向前传播，返回(原始特征列表, 去噪特征列表)
#         """
#         # 原始骨干网络前向传播，获取所有Stage输出
#         x = self.stem(x)
#         outs = []
#         for i, stage in enumerate(self.stages):
#             x = stage(x)
#             if i in self.out_indices:  # 只保留out_indices指定的Stage输出（Stage1-3）
#                 outs.append(x)
#
#         # 对每个输出特征复制并通过去噪模块，生成去噪特征
#         denoised_outs = []
#         if self.use_denoise:
#             for i, feat in enumerate(outs):
#                 # 特征复制：一份用于fusion，一份用于去噪
#                 denoised_feat = self.denoise_modules[i](feat)  # 去噪模块处理复制的特征
#                 denoised_outs.append(denoised_feat)
#
#         # 返回：(原始特征列表, 去噪特征列表)，原始特征用于fusion，去噪特征用于损失计算
#         return tuple(outs), tuple(denoised_outs)
