...
class CocoMetric(BaseMetric):
    ...
    def results2json(self, results: Sequence[dict],
        ...
        for idx, result in enumerate(results):
            ...
            if len(labels) > 0:
                ...
                print(f"[DEBUG results2json] image_id: {image_id}")
                print(
                    f"[DEBUG results2json] labels: {labels[:20].tolist() if hasattr(labels, 'tolist') else list(labels)[:20]}")
                print(f"[DEBUG results2json] max_label: {max_label}, min_label: {min_label}")
                print(f"[DEBUG results2json] len(self.cat_ids): {len(self.cat_ids)}")
                print(f"[DEBUG results2json] self.cat_ids: {self.cat_ids}")

                if max_label >= len(self.cat_ids):
                    print(f"[ERROR] label {max_label} out of range! cat_ids length {len(self.cat_ids)}")
                    print(f"[ERROR] labels with issue: {[l for l in labels if l >= len(self.cat_ids)]}")
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        #         print(f"DEBUG: Tuple length: {len(results[0])}")
        #         for i, item in enumerate(results[0]):
        #             print(f"DEBUG: Tuple[{i}] type: {type(item)}")
        #             if hasattr(item, 'pred_instances'):
        #                 print(f"DEBUG: Found pred_instances in tuple[{i}]")
        #                 pred = item.pred_instances
        #                 print(f"DEBUG: pred_instances has {len(pred)} detections")
        #             elif hasattr(item, 'keys'):
        #                 print(f"DEBUG: Tuple[{i}] keys: {list(item.keys())}")
        # split gt and prediction list
        # 检查results格式
        if len(results) > 0 and isinstance(results[0], tuple):
            print("DEBUG: Detected tuple format results")

            # 统计有效预测
            total_predictions = 0
            for result_tuple in results:
                if len(result_tuple) >= 2:
                    pred_info = result_tuple[1]
                    if 'bboxes' in pred_info:
                        total_predictions += len(pred_info['bboxes'])

            print(f"DEBUG: Total predictions across all images: {total_predictions}")

            if total_predictions == 0:
                logger.warning("No predictions found in any image. Model may need more training.")
                # 返回默认的空指标而不是报错
                return {
                    'bbox_mAP': 0.0,
                    'bbox_mAP_50': 0.0,
