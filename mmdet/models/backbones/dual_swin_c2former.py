...
class Dual_SwinTransformer_C2Former(BaseModule):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(Dual_SwinTransformer_C2Former, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(Dual_SwinTransformer_C2Former, self).train(mode)
        self._freeze_stages()
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            ...
            # print(x_rgb.shape, hw_shape_rgb)
            # print(x_tir.shape, hw_shape_tir)
            ...
            # print(out_rgb.shape, out_hw_shape_rgb)
            # print(x_tir.shape, hw_shape_tir)
            # print(out_tir.shape, out_hw_shape_tir)
            if i == 0:
                x_rgb_out = 0
                x_tir_out = 0
            else:
                c2former = self.c2formers[i - 1]
                x_rgb_out, x_tir_out = c2former(out_rgb, out_tir, out_hw_shape_rgb, out_hw_shape_tir)
            
            # print(x_rgb_out.shape, out_hw_shape_rgb)
            # print(x_tir.shape, hw_shape_tir)

            out_tir = out_tir + x_tir_out
            out_rgb = out_rgb + x_rgb_out
...
class C2Former(nn.Module):
    ...
    def forward(self, vis_x, lwir_x, vis_x_shape, lwir_x_shape):
        B, H_W, C  = vis_x.size()
        H, W = vis_x_shape
        
        vis_x = einops.rearrange(vis_x, 'b (h w) c -> b c h w', h=H, w=W)
        lwir_x = einops.rearrange(lwir_x, 'b (h w) c -> b c h w', h=H, w=W)
        ...
        out_vis = einops.rearrange(out_vis, 'b c h w -> b (h w) c', h=H, w=W)
        out_lwir = einops.rearrange(out_lwir, 'b c w h-> b (h w) c', h=H, w=W)
